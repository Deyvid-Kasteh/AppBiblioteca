...
export default function Romance({ navigation }) {
  ...
  useEffect(() => {
    fetch(
      ...
      .then((resp) => resp.json())
      .then((data) => {
    console.log(data);  // Isso vai imprimir o conteúdo de "data" no console
    setResultadosLivrosRomance(() => data.items);  // Atualiza o estado com "data.items"
});
  ...
  function renderItem({ item }) {
    <View style={styles.resultsContainer1}>
      <Text style={styles.resultsContainer2}>
{item}
      </Text>
      <Image style={styles.resultsContainer3}>
